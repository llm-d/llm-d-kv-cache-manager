...
class StorageOffloadingHandler(OffloadingHandler):
    ...
    def compute_pinned_mb(self,dst_tensors, gpu_blocks_per_file, kv_before_num_blocks, layers_before_num_blocks, num_blocks_idx, safety=1.0, min_mb=32, max_mb=None):
        """Estimate pinned memory size in MB, applying safety and min/max limits."""
        ref = dst_tensors[0]
        per_plane_elems = ref[num_blocks_idx].numel() # elements for blocks: (num_blocks, H, B, D)
        block_elems = per_plane_elems * gpu_blocks_per_file # multiply by blocks per file
        total_elems = block_elems * 2 if kv_before_num_blocks else block_elems
        total_elems = total_elems * len(dst_tensors) if layers_before_num_blocks else total_elems
