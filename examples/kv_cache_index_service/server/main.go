...
func main() {
	...
	if err != nil {
		logger.Error(err, fmt.Sprintf("Failed to listen: %v", *addr))
	}

	indexerSvc, err := setupIndexerService(ctx, defaultConfig())
	if err != nil {
		logger.Error(err, "failed to create indexer service")
	}

	// Setup events pool with ZMQ subscriber
	eventsPool := unit.SetupEventsPool(ctx, indexerSvc.indexer.KVBlockIndex())
	...
	if err := testdata.RunEventsDemo(ctx, indexerSvc.indexer, publisher); err != nil {
