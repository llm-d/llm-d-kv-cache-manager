...
The offline example demonstrates how the KV-Cache handles KV-Events through a dummy ZMQ publisher.
...
1. Initializing the KV-Cache and the dummy publisher
2. Querying the KV-Cache for Pod scores (initially empty)
3. Simulating KV-Events by the publisher for a dummy prompt
4. Querying the KV-Cache for pod scores again (expecting updated scores)

## Online

The online example demonstrates how to deploy the KV-Cache with real-time KV-Events processing and HTTP endpoints for scoring prompts and chat completions.
...
1. Send a long prompt to the KV-Cache (expect no pod scores):
```bash
...
1. Query the KV-Cache again with the same prompt:
```bash
curl -X POST "http://localhost:8080/score_completions" \
  -H "Content-Type: application/json" \
  -d '{"prompt":"'"${TEXT}"'", "model":"'"${MODEL}"'"}' | jq
...
These endpoints allow for scoring rendered chat templates and generating full chat completions using the KV-Cache.
